...
export function AcquisitionBridgePanel({ candidate, acquisition, existingMission }: { candidate: DiscoveryCandidate; acquisition?: RevenueAcquisitionCase; existingMission: boolean }) {
  ...
  return <div data-testid={EXECUTION_MISSION.acquisitionBridge} className="mt-4 space-y-4 border-t border-gborder pt-4">
    ...
    {(!acquisition || acquisition.status === "changes_requested") && <div data-testid={EXECUTION_MISSION.acquisitionDraft} className="rounded-sm border border-gold/30 bg-gold/5 p-3"><div className="text-sm font-semibold text-gold">1. GARUDA drafts the truthful {acquisition?.status === "changes_requested" ? "revision" : "proposal"}</div>{acquisition?.status === "changes_requested" && <div className="mt-2 text-xs text-state-warning">Client requested changes. The earlier handoff cannot be reused; a fresh proposal and Founder approval are required.</div>}<div className="mt-3 grid gap-2 sm:grid-cols-3"><select value={proposalType} onChange={(e) => setProposalType(e.target.value as "application" | "quotation")} className={field}><option value="application">Application</option><option value="quotation">Quotation</option></select>{proposalType === "quotation" && <><input type="number" min="0.01" value={quoteAmount} onChange={(e) => setQuoteAmount(e.target.value)} placeholder="Proposed price" className={field}/><div className="grid grid-cols-2 gap-2"><input value={quoteCurrency} onChange={(e) => setQuoteCurrency(e.target.value)} maxLength={3} className={field}/><input type="number" min="1" max="365" value={deliveryDays} onChange={(e) => setDeliveryDays(e.target.value)} placeholder="Days" className={field}/></div></>}</div><button disabled={!quoteReady || draft.isPending} onClick={() => draft.mutate()} className="mt-3 flex w-full items-center justify-center gap-2 rounded-sm bg-gold px-3 py-2 text-sm font-semibold text-bg disabled:opacity-40"><ShieldCheck size={16}/>Generate grounded {acquisition?.status === "changes_requested" ? "revision" : "proposal"}</button></div>}

    {acquisition && <div className="rounded-sm border border-gborder p-3 text-xs"><div className="flex items-center justify-between gap-2"><span className="uppercase text-state-success">{acquisition.status.replaceAll("_", " ")}</span><span className="text-text_muted">{acquisition.auditTrail.length} chained events</span></div><div className="mt-3 font-semibold text-text_primary">{acquisition.proposal.title}</div><div className="mt-2 text-text_secondary">{acquisition.proposal.summary}</div><div className="mt-3 space-y-1">{acquisition.proposal.deliverables.map((item) => <div key={item}>• {item}</div>)}</div><div className="mt-3 font-mono text-[10px] text-text_muted">Proposal {acquisition.proposal.proposalHash.slice(0, 16)}…</div></div>}
